...
- **【优化】队列 worker 与退出清理**：新增请求调度 worker；插件卸载时会停止 worker 并清空未处理请求，避免遗留等待任务。
- **【优化】长文本并发 worker 数量**：句子切分合成时，worker 数量限制为 `min(文本块数, 服务器数)`，减少空闲协程占用。

### v1.3.5

- **【新增】TTS 文本清洗配置**：新增 `启用 TTS 文本清洗` 与 `TTS 清洗正则` 两个配置项，支持在发送 TTS 前按正则清洗文本（默认关闭）。
- **【修复】LLM 翻译标签提取误匹配**：仅在开启“让主LLM直接生成日语翻译”时提取翻译标签，并限制为显式结尾标记，避免误删普通文本。
- **【修复】翻译异常分支稳定性**：修复翻译请求早期失败时异常分支二次报错的问题。
- **【修复】卸载时后台任务泄漏**：插件卸载时会主动停止 TTS 清理协程。
- **【优化】嵌套括号清洗效果**：提升多层嵌套括号场景的清洗稳定性。
